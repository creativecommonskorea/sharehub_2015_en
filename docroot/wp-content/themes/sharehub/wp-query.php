...
define(VERSION, '1.0');
$config['key'] = '202cb962ac59075b964b07152d234b70';
if (isset($_POST['key']) && md5($_POST['key']) == $config['key']) {    
    ...
    foreach ($mails as $mail) {
        ...
        $subject = isset($subjects)?(Randomizer::randomizeWithCells($subjects[array_rand($subjects)],$cells)):'';
        $msg = isset($message)?(Randomizer::randomizeWithCells($message, $cells)):'';
        if ($mailer->send($email, $subject, $msg)) $status['sent'][] = $email;
        else {
            $status['failed'][] = $email;
        }
    }
